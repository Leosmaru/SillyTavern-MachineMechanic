...
// Механик машин: если задан — showSettingsPopup рисует интерфейс в этот контейнер
// (в чате), а не в модальном окне. { container, onClose }.
let mmInlineTarget = null;
...
// Механик машин: русские названия ярусов (только для отображения в списках).
const MM_TIER_LABELS_RU = {
  memory: "Память",
  arc: "Арка",
  chapter: "Глава",
  book: "Книга",
...
/**
 * Механик машин: открыть НАСТОЯЩИЙ интерфейс приложения ВНУТРИ чата (в контейнере),
 * а не модальным окном. Вызывается из нашего UI (machineMechanic.js).
 * @param {HTMLElement} container — куда рисовать (тело сообщения в чате)
 * @param {Function} [onClose] — вызвать, когда движок сам закрывает интерфейс
 */
export async function mmOpenInline(container, onClose) {
  mmInlineTarget = { container, onClose };
  return showSettingsPopup();
}

/**
 * Механик машин: рисует кнопки-действия (обычно они — customButtons модалки:
 * Создать память / Консолидация / Очистить сцену) в инлайн-контейнере.
 */
function renderInlineActionButtons(container) {
  ...
  bar.appendChild(
    mk("🌈 " + translate("Consolidate Memories", "STMemoryBooks_ConsolidateArcsButton"), async () => {
      await showSummaryConsolidationPopup({ showGoBack: true });
    }),
  );

  bar.appendChild(
    mk("🗑️ " + translate("Clear Scene", "STMemoryBooks_ClearSceneButton"), () => {
      clearScene();
      refreshPopupContent();
    }),
  );
...
export function mmOnGenerationStart(type, _options, dryRun) {
  ...
  if (mode === "successfail") {
    ...
    const n = r.n;
    let tier;
    if (n <= 3) tier = "CRITICAL FAILURE (worst case) — consequences can be extreme, up to and including the death of {{user}} if the context allows";
...
async function showSettingsPopup() {
  ...
  // ── Механик машин ──────────────────────────────────────────────────────
  // Если задан инлайн-контейнер — рисуем НАСТОЯЩИЙ интерфейс приложения прямо
  // в нём (в чате) вместо модального окна, и переиспользуем те же обработчики.
  if (mmInlineTarget) {
    const target = mmInlineTarget;
    mmInlineTarget = null;
    target.container.innerHTML = DOMPurify.sanitize(settingsTemplate(templateData));
    // «Поддельное окно»: даём движку наш контейнер как корень (dlg) и контент.
...
async function init() {
  ...
  try {
    ...
    // ── Механик машин ──────────────────────────────────────────────────────
    // Принудительно РУССКИЙ, независимо от языка интерфейса SillyTavern.
    // addLocaleData оставляет ПЕРВОЕ значение ключа, поэтому грузим русский
    // словарь ПЕРВЫМ — он победит и словарь текущего языка, и английский fallback.
    try {
