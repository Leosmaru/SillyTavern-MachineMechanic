...
- НОВОЕ: 🎬 Режиссёр — **окно при создании NPC** (тумблер, по умолч. вкл). Имя (правится) +
  набросок режиссёра (🌐 транслит) + поле «Промт». Три кнопки внизу, каждая сразу применяет
  и закрывает: «Прокачать сильнее» → мега, «Прокачать» → средняя, «Оставить как есть» → есть
  промт → слабая по промту, нет → набросок как есть. Промт задаёт персонажа заново
  (перебивает набросок). Генерация NPC — с антицензурой (userSysPrompt + UNCENSORED +
  prefill, `trimNames:false`).
