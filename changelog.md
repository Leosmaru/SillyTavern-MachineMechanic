...
### 2026-07-28
- НОВОЕ: 🎬 Режиссёр — **окно проверки NPC** после генерации: промт (редактируемый) + карточка
  (🌐 Яндекс-перевод), кнопки Утвердить / 🔄 Перегенерировать / Отмена (удаляет персонажа и
  снимает событие-приход). Имя правится.
- НОВОЕ: 🎬 Режиссёр — **лорбук по требованию по схеме памяти**: нет привязанного → окно
  «создать (имя любое) / выбрать существующий / отмена». Отмена → персонаж не создаётся.
  Режиссёр и память используют один лорбук (через `chat_metadata`); создаётся `createNewWorldInfo`,
  сразу виден в книжке; после записи редактор WI перечитывается.
- НОВОЕ: 🎬 Режиссёр — **🔄 перегенерация NPC** у имени (то же окно, что при создании).
- ИЗМЕНЕНО: генерация NPC — **всегда English** (миграция `npcPromptVer=2`); промт задаёт персонажа
  целиком, включая **новое имя** по контексту (`NAME:` → переименование записи). Убран `prefill`
  (ломал reasoning-модели GLM-5 → `content:null`). Требование конкретного имени (против ноунеймов).
- ФИКС: `renderNpcTpl` использовал `seed` без параметра (ReferenceError → прокачка молча падала).
- HTML: обновлена справка «Как пользоваться» и «?»-подсказки в панели режиссёра.
- НОВОЕ: 🎬 Режиссёр — **удаление NPC** (🗑 в списке). Удаляет персонажа насовсем: запись из
  лорбука чата + из `World.md` (`ws.npcs`). С подтверждением.
- НОВОЕ: 🎬 Режиссёр — **окно при создании NPC** (тумблер, по умолч. вкл). Имя (правится) +
  набросок режиссёра (🌐 транслит) + поле «Промт» + два режима «Прокачать» / «Прокачать
  сильнее» (подсвечиваются) и кнопка «Применить» снизу. Применить: «сильнее» → мега,
  «Прокачать» → средняя, режим не выбран + промт → слабая по промту, ничего → набросок как
  есть. Промт задаёт персонажа заново (перебивает набросок). Генерация NPC — с антицензурой
  (userSysPrompt + UNCENSORED + prefill, `trimNames:false`).
- УБРАНО: 🎬 Режиссёр — тумблер «Жёсткая директива» (`hardDirective`). Инъекция события
  ролью USER на глубине 0 — уже самый сильный рычаг (персонаж отвечает на реплику
  нарратора), напор поверх был избыточен. Осталась только мягкая `[Narrator]: …`.
- ФИКС: 🎬 Режиссёр — дубль плашки события на малой частоте. Пока прежнее событие не
  израсходовано, режиссёр обновляет мир, но новое событие/NPC не заряжает (эхо/гонка на
  частоте 1). Плюс: не вводит повторно тех, кто уже на сцене; свайп помечает событие
  использованным.
- НОВОЕ: 🎬 Режиссёр — **прокачка карточки NPC** (вручную, кнопка ✨). Короткую запись
  превращает в досье отдельной генерацией по контексту истории и пишет в ту же запись
  лорбука. Уровень «средняя» или по галочке 🚀 «мега» (полное досье). Оба промпта
  прокачки редактируются в «Промпт режиссёра». Правки карточки (✎, клик, возврат по
  имени) сохраняются — контент пишется только при создании записи.
- НОВОЕ: 🎬 Режиссёр — **NPC-реестр** (тумблер, по умолч. выкл). Введённые режиссёром
  персонажи заводятся записью в лорбуке чата (тот же слот, что память; помечены `mm_npc`,
  без флага `stmemorybooks` → саммаризатор игнорит; `constant`, глубина 4, роль SYSTEM).
  Вход — событие `visitor` создаёт/включает запись; выход — поле `npc_remove` в плане
  режиссёра гасит её (`disable`, не удаляя). Возврат по имени — включает ту же запись.
  Нет привязанного лорбука — создаётся через `autoCreateLorebook`. В панели список NPC:
  активные тегом, клик по имени — на сцену/со сцены, ✎ — правка, «+ NPC вручную».
- НОВОЕ: 🎬 Режиссёр — тумблер «**Давать персонажу состояние мира**» (по умолч. выкл):
  на генерации в промпт персонажа тихо подаётся состояние из `World.md` (роль SYSTEM,
  глубина 4, без JSON-хвоста; в лорбук не пишется, память не сканирует).
- Файл `director.js`.
