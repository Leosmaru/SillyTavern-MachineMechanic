...
- НОВОЕ: 🎬 Режиссёр — **окно при создании NPC** (тумблер, по умолч. вкл). Имя (правится) +
  набросок режиссёра (🌐 транслит), поле «Промт» и два **режима** «Прокачать» / «Прокачать
  сильнее» (выделяемые) + кнопка «Применить». Применить: «сильнее» → мега, «Прокачать» →
  средняя, режим не выбран + промт → слабая по промту, ничего не выбрано → набросок как
  есть. Промт задаёт персонажа заново (перебивает набросок). Отмена → как есть. Генерация
  NPC — с антицензурой (userSysPrompt + UNCENSORED + prefill, `trimNames:false`).
