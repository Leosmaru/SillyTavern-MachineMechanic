...
### 2026-07-28
- НОВОЕ: 🎬 Режиссёр — **окно при создании NPC** (тумблер, по умолч. вкл). Когда режиссёр
  вводит нового персонажа, всплывает окно: имя + набросок (🌐 транслит), поле «Промт
  (пусто = прокачает сам)» и кнопки «Прокачать» / «Прокачать сильнее». Закрыл — базовая
  карточка как есть. Промт идёт поверх контекста. Генерация NPC — с антицензурой
  (системный промпт пользователя + UNCENSORED + prefill, `trimNames:false`).
