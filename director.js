...
function pageHtml() {
    ...
    <details class="mmobj-help">
        ...
        <div>
            Режиссёр — скрытый «мастер сцены». Раз в N ответов смотрит на разговор,
            обновляет мир (место, время, инвентарь, факты) и по необходимости вбрасывает
            событие, которое персонаж вплетает в свой ответ.
            <ul>
                <li><b>Тумблер 🎬 в шапке</b> — включить/выключить режиссёра.</li>
                <li><b>Частота</b> — раз в N ответов запускать режиссёра (0 = выкл).</li>
                <li><b>Стиль повествования</b> — как режиссёр описывает мир и события.</li>
                <li><b>Показ события</b> — «сразу» или «анонс» (событие следующей генерацией).</li>
                <li><b>Состояние мира персонажу</b> — тихо вшивать World.md в промпт (SYSTEM, глубина 4).</li>
                <li><b>Промпт режиссёра</b> — что спрашивать у ИИ (строгий JSON); там же промпты прокачки NPC.</li>
                <li><b>Мир (World.md)</b> — состояние мира; файл этого чата, удаляется с чатом.</li>
            </ul>
            <b>🎭 NPC-реестр</b> (тумблер): режиссёр вводит персонажей событием, они живут записями
            в лорбуке чата (тот же, что память; без флага <code>stmemorybooks</code> — саммари их не трогает;
            constant, глубина 4). Генерация NPC — на English, с антицензурой.
            <ul>
                <li>Нет лорбука → окно «создать / выбрать существующий» (схема памяти, имя любое). Отмена → персонаж не создаётся.</li>
                <li>Появился → окно: имя + набросок (🌐 транслит) + <b>промт</b> + режимы <b>«Прокачать»/«Прокачать сильнее»</b> + «Применить».</li>
                <li>Промт задаёт персонажа целиком (новое имя — по контексту и промту). Пусто = как набросал режиссёр.</li>
                <li>После генерации — окно проверки: промт + карточка (🌐). <b>Утвердить</b> / <b>🔄 Перегенерировать</b> / <b>Отмена</b> (удалить + снять событие). Имя правится.</li>
                <li>В списке у имени: клик — на сцену/со сцены, <b>✨/🚀</b> прокачать, <b>🔄</b> окно как при создании, <b>✎</b> правка, <b>🗑</b> удалить.</li>
                <li><b>Мега-прокачка</b> (галочка) — кнопка ✨ делает полное досье вместо среднего.</li>
            </ul>
            <small>🎬 в шапке — вкл. «Стригерить событие» / 🚪 ГОСТЬ — разово гарантированно. Пресет меняет ритм.</small>
        </div>
    </details>
    ...
    <div class="objective_block objective_block_control flex1 flexFlowColumn" style="margin-top:8px;">
        ...
        <label class="checkbox_label" style="margin-top:6px;"><input id="mmdir-showworld" type="checkbox"> Давать персонажу состояние мира <span class="mm-help-q" title="На генерации в промпт персонажа тихо (роль SYSTEM, глубина 4, как дневник) подаётся текущее состояние из World.md без служебного JSON. В лорбук не пишется, память не сканирует.">?</span></label>
    ...
    <label class="checkbox_label"><input id="mmdir-npc" type="checkbox"> NPC-реестр (режиссёр вводит/выводит персонажей) <span class="mm-help-q" title="Введённые режиссёром персонажи заводятся записями в лорбуке чата (том же, что память; без флага stmemorybooks — саммари их не трогает; constant, глубина 4). Вход — событие visitor, выход — поле npc_remove. Нет лорбука → окно создать/выбрать. Генерация NPC — на English.">?</span></label>
    ...
    <label class="checkbox_label" style="margin-top:4px;"><input id="mmdir-npc-popup" type="checkbox"> Окно при создании NPC (промт + прокачка) <span class="mm-help-q" title="Когда режиссёр вводит нового персонажа, всплывает окно: имя + набросок (🌐 транслит) + поле промта + режимы «Прокачать»/«Прокачать сильнее» + «Применить». Промт задаёт персонажа (в т.ч. новое имя). После генерации — окно проверки (Утвердить/Перегенерировать/Отмена).">?</span></label>
    <label class="checkbox_label" style="margin-top:4px;"><input id="mmdir-npc-mega" type="checkbox"> 🚀 Мега-прокачка — полное досье <span class="mm-help-q" title="Кнопка ✨ «Прокачать» у NPC делает полное досье (Описание/Личность/Предыстория/Манера речи/Роль/примеры реплик) вместо среднего. Оба промпта редактируются в «Промпт режиссёра».">?</span></label>
